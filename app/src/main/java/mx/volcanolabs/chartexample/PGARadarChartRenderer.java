...
public class PGARadarChartRenderer extends LineRadarRenderer {

    protected PGARadarChart mChart;
    ...
    public PGARadarChartRenderer(PGARadarChart chart, ChartAnimator animator,
        ...
        for (int i = 0; i < mChart.getData().getDataSetCount(); i++) {
            ...
            for (int j = 0; j < dataSet.getEntryCount(); j++) {
                ...
                if (dataSet.isDrawValuesEnabled()) {
                    ...
                    drawValueCircle(c, pOut.x, pOut.y, dataSet.getValueTextColor(j));
                }
    ...
    public void drawValueCircle(Canvas c, float x, float y, int color) {
        mValuePaint.setColor(color);
        c.drawCircle(x, y, 5f, mValuePaint);
    }
    ...
    protected void drawWebCircles(Canvas canvas) {
        MPPointF center = mChart.getCenterOffsets();

        mWebPaint.setStyle(Paint.Style.FILL);
        mWebPaint.setColor(Color.parseColor("#8E8E8E"));
        mWebPaint.setAlpha(10);

        float radius = mChart.getRadius();
        double radiusCircleOffset = radius * 0.20;

        for (int i = 0; i < 5; i++) {
            canvas.drawCircle(center.x, center.y, radius, mWebPaint);
            radius -= radiusCircleOffset;
        }
